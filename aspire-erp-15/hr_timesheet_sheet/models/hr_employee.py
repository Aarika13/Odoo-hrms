    ...
    def _check_company_id(self):
        for rec in self.sudo().filtered("company_id"):
            for field in [
                ...
                if ( rec.company_id and field.company_id and rec.company_id != field.company_id):
                    timesheet_ids = rec.env["hr_timesheet.sheet"].search(
                            [
                                ("employee_id", "=", rec.id),
                                ("company_id", "!=", rec.company_id.id),
                                ("company_id", "!=", False),
                    ...
                    timesheet_ids.company_id = rec.company_id.id
                if (
                    rec.company_id
